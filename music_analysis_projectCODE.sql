-- QUES1 who is senior most employee based on job title
select * from employee
order by levels desc
limit 1

-- QUES2 which country has most invoice
...
--Q9 Find how much amount spent by each customer on best selling artist? Write a query to return customer name, artist name and total spent (cost=unitprice*quantity)
...
select c.first_name,c.customer_id,c.last_name,bsa.artist_name, SUM(il.unit_price*il.quantity) AS amount_spent
FROM invoice i
...
row_number() over (partition by billing_country order by sum(total) desc)
	 ...
	 select * from customer_country where row_number<=1